...
export const useSounds = () => {
  ...
  const [playLevel2, { stop: stopLevel2 }] = useSoundLibrary(mp3.levelTwo, { volume: 0.5 });
  const [playSwichLevel] = useSoundLibrary(mp3.nextLevel, { volume: 0.5 });
  const [playShoot] = useSoundLibrary(mp3.bang, { volume: 0.7 });
  const [playBoom] = useSoundLibrary(mp3.boom, { volume: 0.7 });
  const [playEnemyShoot] = useSoundLibrary(mp3.bossShoot, { volume: 0.7 });
